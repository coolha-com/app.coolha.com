...
| `src/app/(APP)/chat/`                   | 已有         | 可作为持仓概览、USDC 余额、交易入口的承载页               |
...
| `src/app/(APP)/chat/page.tsx`           | 已存在        | 改造成 Base 资产总览、USDC 中心仪表板                 |
...
| 5. Dashboard 升级  | 展示 Base 地址、网络、USDC 余额、重点资产                                        | `src/app/(APP)/chat/*`          |
